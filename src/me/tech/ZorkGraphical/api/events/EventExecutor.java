package me.tech.ZorkGraphical.api.events;
...
public class EventExecutor {
    ...
    public <T extends IEvent> T executeEvent(T event, int i) {
        ...
        if (handlers == null) {
            if (this.debug)
                Zork.getInstance().println("Event " + event.getClass().getSimpleName() + " has no handlers.");
            return event;
        }
        if (this.debug)
            Zork.getInstance().println("Event " + event.getClass().getSimpleName() + " has " + handlers.size() + " handlers.");
    ...
    public void registerListener(final EventListener listener) {
        //Zork.getInstance().println("Register event listener: " + listener);

        if (registeredListeners.contains(listener)) {
            //Zork.getInstance().println("Listener already registred: " + listener);
            return;
        }
        ...
        for (final Method method : methods) {
            ...
            if (!method.getReturnType().equals(void.class)) {
                //Zork.getInstance().println("Ignoring method due to non-void return: " + method.getName());
                continue;
            }
            ...
            if (IEvent.class.isAssignableFrom(param)) {
                ...
                //Zork.getInstance().println("Add listener method: " + method.getName() + " for event " + realParam.getSimpleName());
